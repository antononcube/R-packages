    ...
    fluidPage(
      ...
      fluidRow(
        ...
        column( 6,
                tabsetPanel(
                  ...
                  tabPanel( "Consumed items list",
                            ...
                            tabsetPanel(
                              tabPanel( "short", dataTableOutput("itemList") ),
                              tabPanel( "extended", dataTableOutput("itemListData") )
                            )
                  )
                )
        ),
        ...
        column( 3,
                ...
                dataTableOutput("significanceFactors")
        )
      ),
      ...
      fluidRow(
        column( 9,
                h4("Recommendations"),
                tabsetPanel(
                  tabPanel( "main", dataTableOutput("recs") ),
                  tabPanel( "proofs", dataTableOutput("recsProofs") ),
        ...
        column( 3,
                tabsetPanel(
                  tabPanel( "Profile",
                            ...
                            h4("Profile"),
                            dataTableOutput("uprofile") ),
                  tabPanel( "Detailed proof",
                            h4("Detailed proof"),
                            dataTableOutput("uproof") )
                )
        )
      )
    )
...
SMRMakeServerFunction <- function( itemSMR, itemData, itemDataColNames = NULL, itemDataIDColName = NULL, searchColName = NULL, itemListIDsSplitPattern = "\\W" ) {
  ...
  shinyServer(function(input, output, session) {
    ...
    output$significanceFactors <- renderDataTable({ datatable({
    ...
    output$itemList <- renderDataTable({ datatable({
    ...
    output$itemListData <- renderDataTable({ datatable({
      extendedItemData()
    }, rownames = FALSE, filter = 'top', options = list(pageLength = 12, autoWidth = FALSE) ) })

    ## Using simple title search
    output$view <- renderDataTable({ datatable({
    ...
    output$uprofile <- renderDataTable({ datatable({
      itemSMR$M <- SMRApplyTagTypeWeights( itemSMR, tagTypeSFactors() )
    ...
    output$uproof <- renderDataTable({ datatable({
