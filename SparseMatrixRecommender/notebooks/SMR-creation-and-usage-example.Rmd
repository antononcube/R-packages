...
```{r, eval=F}
library(devtools)
install_github( repo = "antononcube/R-packages", subdir = "SparseMatrixRecommender" )
```
...
recs %>% dplyr::inner_join( dfTitanic, by = c("Item" = "id"))
```

```{r}
recs2 <- SMRRecommendationsByProfileDF( smr = smr2, profile = prof[, 1:2], nrecs = 12 )
recs2 %>% dplyr::inner_join( dfTitanic, by = c("Item" = "id"))
```
