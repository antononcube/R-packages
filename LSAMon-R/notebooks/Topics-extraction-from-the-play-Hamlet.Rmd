...
lsaObj <-
  ...
  LSAMonMakeDocumentTermMatrix( stemWordsQ = TRUE, stopWords = NULL ) %>% 
...
```{r}
stopwords::stopwords(language = "Norwegian")
```
```{r, eval=F}
textHamlet
```
...
```{r}
lattice::histogram(cs)
```

```{r}
dim(lsaObj %>% LSAMonTakeDocumentTermMatrix)
colnames(lsaObj %>% LSAMonTakeDocumentTermMatrix)
```

```{r}
as.matrix((lsaObj %>% LSAMonTakeDocumentTermMatrix)[,c("queen", "hamlet", "king")])
```
...
```{r}
lsaObj2 <- lsaObj %>% LSAMonTopicRepresentation( tags = tags, minThreshold = 0.005 )
...
lsaObj2 <-
  ...
  LSAMonApplyTermWeightFunctions( "IDF", "None", "Cosine" ) %>% 
  LSAMonExtractTopics( numberOfTopics = 20, minNumberOfDocumentsPerTerm = 10, method = "SVD")
```

```{r}
qmat <- 
  lsaObj2 %>% 
  LSAMonRepresentByTopics( query = textHamlet[c(6,8,10,12)] ) %>% 
  LSAMonTakeValue
```

```{r}
qmat <- t( SparseMatrixRecommender::SMRApplyTermWeightFunctions( t(qmat), "None", "None", "AbsMax") )
cat("\n")
as.matrix(qmat)
```

```{r}
qmat2 <- lsaObj2 %>% LSAMonTakeW
qmat2 <- qmat2[c("id.6","id.8","id.10","id.12"),]
qmat2 <- t( SparseMatrixRecommender::SMRApplyTermWeightFunctions( t(qmat2), "None", "None", "AbsMax") )
cat("\n")
as.matrix(qmat2)
```
