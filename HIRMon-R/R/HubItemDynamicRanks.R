...
##=======================================================================================
## Version 0.4
## This implementation started as re-implementation of a Mathematica implementation of HIDRA.
...
#' Remove zero entries.
#' @description Remove the explicit zero entries from a sparse matrix.
#' @param x A sparse matrix.
#' @param tol A non-negative number to be used as tolerance for checking
#' if an entry \code{x[i,j]} should be considered to be zero.
#' @details Just calls \code{\link{Matrix::drop0}}.
#' @export
RemoveZeroEntries <- function( x, ....) {
  Matrix::drop0(x)
}
...
#' with row-names and col-names corresponding to the hub and item ID's.
...
#' @param tol Tolerance for \code{\link{PowerMethod}}.
...
HubItemRanks <- function( hubsAndItemsMat, hubIDs, hubTags, tagSets,
  ...
  rankVecs <- purrr::map( tagSets,
                          function(ts) {
                            ...
                            ## Find the ranks.
                            res <- PowerMethod( mat = hubsAndItemsMat, bvec = bvec, alpha = sumFactor, maxSteps = maxSteps, tol = tol )
  ...
  purrr::map( 1:length(tagSets), function(i) {
    list( TagSet=tagSets[[i]], HubRankVector=rankVecs[[i]]$HubRanks, ItemRankVector=rankVecs[[i]]$ItemRanks )
  })
