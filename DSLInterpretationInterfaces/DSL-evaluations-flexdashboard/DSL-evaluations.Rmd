...
consURL <- 
  reactive({
    ...
    } else if( input$translatorType == "ast") {
      resURL <- paste0( input$webServiceURL, "/", "translate", "/", "ast", "/")
    ...
    if( !( input$translatorType %in% c("numeric", "ast") ) ) {
      resURL <- paste0( resURL, input$progLang, "/")
    } 
...
```{r}
renderPrint( expr = DSLWebServiceInterpretationURL(textUserCommands(), url = consURL()) )
```
...
observeEvent(input$submitButton, {
  ...
  if( nchar(input$userCommands) == 0) { 
    ...
    dwsResp <- InterpretByDSLWebService(textUserCommands(), url = consURL())
