...
test_that("Profile data frame", {
  ...
  expect_s3_class( prof0 <- smrObj %>% SMRMonProfile( history = c("1001"), tagTypesQ = FALSE ) %>% SMRMonTakeValue, "data.frame" )
  expect_s3_class( prof1 <- smrObj %>% SMRMonProfile( history = c("1001"), tagTypesQ = TRUE ) %>% SMRMonTakeValue, "data.frame" )
  ...
  expect_s3_class( prof2 <- smrObj2 %>% SMRMonProfile( history = c("id.10"), tagTypesQ = FALSE ) %>% SMRMonTakeValue, "data.frame" )
  expect_s3_class( prof3 <- smrObj2 %>% SMRMonProfile( history = c("id.10"), tagTypesQ = TRUE ) %>% SMRMonTakeValue, "data.frame" )
